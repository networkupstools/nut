...
- Added generation of "driver" actions list for USB devices supported
  by NUT to help IPS (aka `pkg(5)`) packaging for Solaris/illumos: the
  resulting `scripts/Solaris/nut-usb-driver.p5m.include` file can be
  used by IPS packaging recipes to enhance the package which delivers
  USB-capable NUT drivers.
