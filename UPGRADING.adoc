...
- The `sms_ser` driver changed the meaning of some commands and values
  [issue #1987]:
  * `test.battery.start.quick` used to start a test that runs until the
    battery is low; it now runs a 10 second test. Use the new
    `test.battery.start.deep` for the former behavior.
  * `shutdown.return` (also used to power off the load at the end of a
    system shutdown) and `shutdown.reboot` now send their delays in the
    format used by the vendor software. Previously `shutdown.return` sent
    no usable delays, and the `shutdown.reboot` delay was mangled (64 was
    sent as `0x40FF`), and converts seconds to the time unit of the UPS,
    so the timing seen with your device may change. See
    the `offdelay`, `ondelay` and `rebootdelay` settings in
    linkman:sms_ser[8]. A malformed `rebootdelay` in `ups.conf` is now fatal
    instead of being silently ignored. If a shutdown or test which worked
    before does not work now, the `legacydelays` flag restores the old
    format; please report such a device.
  * Replies from the UPS are now checked for their checksum. If a device
    which worked before is rejected, the `novalidate` flag turns the check
    off; please report such a device.
  * `ups.status` may now hold several values (e.g. `OB LB`) where only one
    was reported before, and line-interactive models no longer report
    `BYPASS`; drop any workarounds which translated that value. With a low
    battery properly reported, `upsmon` will now shut the system down where
...
- The common configuration and protocol parser now accepts literal `#`
  ...
  still starts a comment. Existing `\#` escapes remain supported and are
  still needed when sharing configuration files with older NUT versions.
  Encoded protocol output continues to escape hashes for older peers;
  default `upsc` text output is unchanged. [issues #607, #1306, PR #3633]
...
- Correct malformed numeric values in `upsd.conf` when upgrading: `MAXAGE`,
  ...
  and ignored, retaining the previous setting. Valid decimal values, leading
  zeroes and trailing whitespace remain accepted. [issue #676, PR #3631]
...
- The NUT-Monitor GUI client application has historically branched from an
  implementation with Python 2 syntax and GTK 2 backend for GUI, into another
  script with Python 3 syntax and QT5 backend, and later another with QT6.
  Those two latter scripts have now been re-merged into one with Python 3
  syntax and support for both QT5 and QT6 backends, one selected at run-time
  by the executable script name. Packaging-wise this should remain identical
  to earlier releases (also for distributions which delivered more than one
  implementation), however dependency rules can be now collapsed to requiring
  any of the QT5/QT6 backends for essentially the same client script. Delivery
  of the two filenames for same contents (to pick the backend at run-time) may
  also be optimized with symbolic or hard links in packaging. [PR #3596]

- Manual page name for `generic_gpio_libgpiod` driver was fixed to match its
  name (previously the document base file name could have been `generic_gpio`).
  This can impact packaging recipes. [PR #3693]
...
- Related to the above, `libupsclient` will remove the exported symbol for
  ...
  `upscli_upslog_set_debug_level()` and `upscli_upslog_get_debug_level()`
  methods. [PR #3330]
