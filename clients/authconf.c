...
static void handle_authconf_args(size_t numargs, char **arg, int global_scope)
{
	...
	if (current_section) {
		...
		/* Initial spec says global-scope includes may modify
		 * global default items, as well as define new sections
		 * or overlay items in existing sections.
		 * This implementation handles this by remembering the
		 * most-recent "current_section" state.
		 */
