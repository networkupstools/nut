...
#recovery# zpool import
 ...
 config:
        rpool ONLINE
           vda ONLINE
----
* RECOMMENDED: Double-check that `ashift=12` (4KB sectors) setting is applied.
  Storage may subtly misbehave (from being slow to getting corrupted) when
  using default `ashift=9` (512B sectors) on disks with 4KB actual sectors,
  especially under stress such as CI builds. It may be worthwhile to recreate
  the ZFS pool if it was prepared in a "wrong" manner.
...
* DigitalOcean rescue CD is Ubuntu 18.04 based, it has an older ZFS version
  ...
  * With DigitalOcean VMs it is recommended to explicitly assign `ashift=12`
    when creating ZFS pools on systems which support setting this explicitly.
    By default the storage sector size is guessed, and may end up using `9`
    (512 bytes) leading to read-modify-write cycles in the underlying storage
    layers, causing slower I/O and even possible data corruption.
  * Usually you can check a resulting ZFS pool's `ashift` value with
    `zpool get ashift zroot`; however for a guessed value `0` may be reported.
    In this case, `zdb -C -e zroot | grep ashift` is a more certain way.
  * WARNING: The `ashift` value can only be assigned during pool creation!
