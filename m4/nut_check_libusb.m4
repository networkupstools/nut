...
dnl and set NUT_LIBUSB_CFLAGS and NUT_LIBUSB_LDFLAGS. On failure, set
...
if test -z "${nut_have_libusb_seen}"; then
   ...
   NUT_LIBUSB_CFLAGS=`libusb-config --cflags 2>/dev/null`
   if (test "$?" != "0")
   then
	AC_MSG_RESULT(not found)
	nut_have_libusb=no
   else
	AC_MSG_RESULT(${NUT_LIBUSB_CFLAGS})
   fi

   AC_MSG_CHECKING(for libusb libs)
   NUT_LIBUSB_LDFLAGS=`libusb-config --libs 2>/dev/null`
   if (test "$?" != "0")
   then
	AC_MSG_RESULT(not found)
	nut_have_libusb=no
   else
	AC_MSG_RESULT(${NUT_LIBUSB_LDFLAGS})
   fi
