...
if test -z "${nut_have_avahi_seen}"; then
	...
	], [CFLAGS="`pkg-config --silence-errors --cflags avahi-core avahi-client 2>/dev/null || echo "-I/usr/local/include -I/usr/include -L/usr/local/lib -L/usr/lib"`"])
	...
	], [LIBS="`pkg-config --silence-errors --libs avahi-core avahi-client 2>/dev/null || echo "-lavahi-core -lavahi-client"`"])
	AC_MSG_RESULT([${LIBS}])
