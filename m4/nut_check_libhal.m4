...
dnl and set NUT_LIBHAL_CFLAGS and NUT_LIBHAL_LDFLAGS. On failure, set
...
if test -z "${nut_have_libhal_seen}"; then
   ...
   AC_MSG_CHECKING(for libhal cflags via pkg-config)
	NUT_LIBHAL_CFLAGS=`pkg-config --silence-errors --cflags hal`
   if (test "$?" != "0")
   then
	AC_MSG_RESULT(not found)
	nut_have_libhal=no
   else
	AC_MSG_RESULT(${NUT_LIBHAL_CFLAGS})
   fi

   AC_MSG_CHECKING(for libhal ldflags via pkg-config)
   NUT_LIBHAL_LDFLAGS=`pkg-config --silence-errors --libs hal`
   if (test "$?" != "0")
   then
	AC_MSG_RESULT(not found)
	nut_have_libhal=no
   else
	AC_MSG_RESULT(${NUT_LIBHAL_LDFLAGS})
   fi
   ...
   if test "${nut_have_libhal}" != "yes"; then
      ...
      NUT_LIBHAL_CFLAGS="-DDBUS_API_SUBJECT_TO_CHANGE -I/usr/include/hal -I/usr/include/dbus-1.0 -I/usr/lib/dbus-1.0/include"
      NUT_LIBHAL_LDFLAGS="-lhal -ldbus-1 -lpthread"
      
      CFLAGS="${NUT_LIBHAL_CFLAGS}"
      CPPFLAGS="${NUT_LIBHAL_CFLAGS}"
      LDFLAGS="${NUT_LIBHAL_LDFLAGS}"
