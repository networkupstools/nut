...
[
    ...
    AC_MSG_NOTICE([=======])
    UNITTEST_AX_REALPATH_EXPECT([/tmp/q/x], [/tmp/q/x])
    UNITTEST_AX_REALPATH_EXPECT([/tmp/Q/x], [/tmp/q/x])
    UNITTEST_AX_REALPATH_EXPECT([/tmp/Q/x/qwe], [/tmp/q/x/qwe])
    AC_MSG_NOTICE([=======])
    AC_MSG_NOTICE([### Should not have access to next file (does not exist) in a readable dir])
    UNITTEST_AX_REALPATH_EXPECT([/tmp/q/x/absent], [/tmp/q/x/absent])
    AC_MSG_NOTICE([=======])
