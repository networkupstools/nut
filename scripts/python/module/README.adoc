...
class PyNUTClient :
  def __init__( self, host='127.0.0.1', port=3493,
    ...
    use_ssl=False, ssl_context=None ) :
...
the NUT `STARTTLS` protocol command. In this case, an `ssl_context` can
also be provided (as an `ssl.SSLContext` object) to configure the SSL/TLS
parameters (such as CA certificates, client certificates, or selecting
the underlying library implementation if the Python `ssl` module supports
it). By default, a default context is created if `use_ssl=True` and no
context is provided.
