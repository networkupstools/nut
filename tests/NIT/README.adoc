...
:; sleep 5 ; while ! [ -f ./tests/NIT/tmp/etc/NIT.env-sandbox-ready ] ; do sleep 1 ; done
...
After building `dummy-ups`, `upsd` and `upsc`, run the optional repeater
protocol-fault tests with `make -C tests/NIT check-NIT-repeater` from the
build directory. This requires Python 3 and uses only its standard library.
The test runs disposable loopback endpoints and checks failed refreshes,
incomplete variable lists, repeated recovery, strict and relaxed startup,
and unaffected dummy modes through the real driver, server and client.
It does not use hardware or send power commands. Temporary runtime and log
directories are printed when the test starts.

Caching!
--------

Like the `ci_build.sh`, `nit.sh` can make use of a persistent cache directory
to store generated certificates and speed up repetitive runs of the `configure`
script on developer or CI farm machines when the script sources remain the same.

They can be stored outside of the build area(s), under `${CI_CACHE_NUT_BASEDIR}`
(default: `${HOME}/.cache/nut-ci`) if `DO_USE_NUTCI_CACHE=yes` as provided by
caller (e.g. via `~/.profile` of a developer) or guessed from environment
...
`DO_CLEAN_NIT_TESTCERT_CACHE_BEFORE` to not impact `ci_build.sh`).

You can also always nuke the `${CI_CACHE_NUT_BASEDIR}` directory, or the
`NIT_CERT_*` subdirectories and stale lock files under it.
It would likely be needed from time to time anyway, as there is
now no automated clean-up of no longer relevant caches.
