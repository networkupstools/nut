...
Note there is also a separate `DO_USE_NUTCI_CACHE_DEBUG=yes` toggle, which
aims to keep a copy of not only the `config.cache`, but also of `config.log`
and `config.h` files from a first run with new cache hash for troubleshooting.
