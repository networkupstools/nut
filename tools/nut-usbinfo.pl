...
# Solaris/illumos IPS aka pkg(5) "driver" actions
my $output_pkg5ips_driver_actions="$TOP_BUILDDIR/scripts/Solaris/nut-usb-driver.p5m.include.in";
...
sub gen_usb_files
{
	...
	my $out_pkg5ips_driver_actions = do {local *OUT_PKG5IPS_DRIVER_ACTIONS};
	open $out_pkg5ips_driver_actions, ">$output_pkg5ips_driver_actions" || die "error $output_pkg5ips_driver_actions : $!";
	print $out_pkg5ips_driver_actions "# This file is generated by the Network UPS Tools package and intends to aid in IPS packaging\n";
	print $out_pkg5ips_driver_actions "# and lists NUT USB VendorID:ProductID pairs supported by some USB-capable driver\n";
	print $out_pkg5ips_driver_actions "# Homepage: https://www.networkupstools.org/\n\n";
	print $out_pkg5ips_driver_actions "driver name=ugen \\\n";
	...
	foreach my $vendorId (sort { lc $a cmp lc $b } keys  %vendorName)
	{
		...
		foreach my $productId (sort { lc $a cmp lc $b } keys %{$vendor{$vendorId}})
		{
			...
			# Solaris/illumos IPS aka pkg(5) "driver" actions entry
			print $out_pkg5ips_driver_actions "\talias=\"usb" . removeHexPrefix($vendorId) . "," . removeHexPrefix($productId) . ".*\" \\\n";
	...
	# Solaris/illumos IPS aka pkg(5) "driver" actions footer
	print $out_pkg5ips_driver_actions "\tperms=\"* 0664 root \@RUN_AS_GROUP\@\"\n";
}
