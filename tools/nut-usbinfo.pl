...
sub gen_usb_files
{
	...
	# UPower file header
	my $outputUPower = do {local *OUT_UPOWER};
	open $outputUPower, ">$outputUPower" || die "error $outputUPower : $!";
	print $outputUPower '##############################################################################################################'."\n";
	...
	my $outputDevScanner = do {local *OUT_DEV_SCANNER};
	open $outputDevScanner, ">$outputDevScanner" || die "error $outputDevScanner : $!";
	...
	foreach my $vendorId (sort { lc $a cmp lc $b } keys  %vendorName)
	{
		...
		if ($upowerVendorHasDevices) {
			print $outputUPower " UPOWER_BATTERY_TYPE=ups\n";
			if ($vendorName{$vendorId}) {
				print $outputUPower " UPOWER_VENDOR=".$vendorName{$vendorId}."\n";
			}
		}
