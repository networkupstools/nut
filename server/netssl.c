...
static int ssl_error(SSL *ssl, ssize_t ret)
{
	...
	if (ret >= INT_MAX) {
		upslogx(LOG_ERR, "ssl_error() ret=%" PRIssize " would not fit in an int", ret);
		return -1;
	}
	...
	case SSL_ERROR_WANT_READ:
		upsdebugx(1, "ssl_error() ret=%" PRIssize " SSL_ERROR_WANT_READ", ret);
		break;

	case SSL_ERROR_WANT_WRITE:
		upsdebugx(1, "ssl_error() ret=%" PRIssize " SSL_ERROR_WANT_WRITE", ret);
		break;
	...
	case SSL_ERROR_SYSCALL:
		if (ret == 0 && ERR_peek_error() == 0) {
			...
			upsdebugx(1, "ssl_error() ret=%" PRIssize " SSL_ERROR_SYSCALL", ret);
		}
		break;

	default:
		upsdebugx(1, "ssl_error() ret=%" PRIssize " SSL_ERROR %d", ret, e);
		ssl_debug();
	}
...
ssize_t ssl_write(nut_ctype_t *client, const char *buf, size_t buflen)
{
	...
	upsdebugx(5, "ssl_write ret=%" PRIssize "", ret);
