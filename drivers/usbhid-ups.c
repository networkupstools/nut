	...
	int *flattened)
{
	...
	*alias_path = *src_path;
	...
	/* Some devices expose "UPS.PowerConverter.Input.*" (or similar nested forms)
	 * while current subdriver tables often expect "UPS.Input.*" and siblings.
	 * Keep original path, but add a flattened alias branch. */
...
static int winhid_canonicalize_parsed_report_desc(HIDDesc_t *desc)
{
	size_t i;
	...
	for (i = 0; i < original_nitems; i++) {
		...
		if (ps_idx >= 0) {
			/* Expected by APC-like mappings: UPS.PowerSummary.PresentStatus.* */
			if (has_ups
			 && ps_idx > 0
			...
			/* Map known status leaves from 0x84xx aliases to canonical 0x85xx. */
