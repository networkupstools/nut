 ...
 * USB, or an optional RMS-MODBUS01B (RS-485) adapter. Currently,
 * only RS-232 is supported, you must use connect UPS2000 to your
 * computer via RS-232, either directly or using an USB-to-RS-232
 * converter supported by your Linux or BSD kernel. You cannot use
 * USB or RS-485.
 *
 ...
 * USB-CDC serial device. Unfortunately, the USB serial device does
 * not respond to requests, it always times out. It seems that some
 * device-specific registers on RX21V1410 USB-serial converter must
 * be initialized for this particular hardware configuration used in
 * UPS2000. Nothing can be done within NUT, the kernel needs a driver,
 * and anyone who wants to work on the kernel driver must figure out
 * which registers should be initialized for proper communication.
 *
...
#define DRIVER_NAME	"NUT Huawei UPS2000 (1kVA-3kVA) RS-232 driver"
...
/*
 * "ups.shutdown.reboot": shutdown as soon as possible. Restart
 * after an "ondelay". In our implementation, it's like "ups.
 * shutdown.return", except that a minimum delay is used.
 */
static int ups2000_instcmd_shutdown_reboot(const uint16_t reg)
{
	int r;
...
/*
 * "ups.shutdown.reboot.graceful": shutdown after an "offdelay",
 * Restart after an "ondelay".
 *
 * Note: In our implementation, it's a synonym of "ups.shutdown.
 * return". That is, it won't reboot unless there's grid power,
 * or until the grid power is restored.
 */
static int ups2000_instcmd_shutdown_reboot_graceful(const uint16_t reg)
{
	int r;
...
static int ups2000_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (retry_status == RETRY_ENABLE &&
			...
			upslogx(LOG_INFO, "Battery status has a non-fatal read failure. Retrying... ");
			sleep(1);
			continue;
		}
