...
/* Number of batteries that make a pack */
static int	battery_number;
...
/* Alarm/status buffer */
static char	alarm_buf[SMALLBUF],
		status_buf[SMALLBUF];


/* == Support functions == */
static int	voltronic_protocol(void);
static int	voltronic_fault(void);
...
static int	voltronic_check(void);
static int	voltronic_eco(void);
static int	voltronic_bypass(void);
static int	voltronic_batt_numb(void);
static int	voltronic_batt_packs(void);
static int	voltronic_p31b(void);
static int	voltronic_p31g(void);
...
static int	voltronic_vendor(void);
static int	voltronic_ratings3(void);
static int	voltronic_outlet(const int outlet_numb, const int init);
static int	voltronic_batt_low(void);
static int	voltronic_multi(void);
static int	voltronic_load_level(void);
static int	voltronic_phase(void);
static int	voltronic_parallel(void);
static int	voltronic_qbdr(void);
...
static int	voltronic_battery(void)
{
	const struct {
		...
		{ "battery.number",	"%.0f",	strtod },	/* Number of batteries that make a pack */
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for battery informations rejected by the UPS", __func__);
		return -1;
	}
	...
	for (i = 0, val = strtok_r(buf+1, " ", &last); battery_status[i].var; i++, val = strtok_r(NULL, " \r\n", &last)) {
		...
		if (i != 1) {
			/* Battery runtime (i = 4) is reported by the UPS in minutes, NUT expects seconds */
			dstate_setinfo(battery_status[i].var, battery_status[i].fmt, battery_status[i].conv(val, NULL)*(i == 4 ? 60 : 1));
		} else {
			/* There's no NUT variable for battery number */
...
static int	voltronic_protocol(void)
{
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for protocol rejected by the UPS", __func__);
		return -1;
	}
	...
	if (strspn(prot, "0123489") != strlen(prot)) {
	...
	case 0:
		...
		dstate_setinfo("ups.firmware.aux", "P00");
		break;

	case 1:

		dstate_setinfo("ups.firmware.aux", "P01");
		break;

	case 2:

		dstate_setinfo("ups.firmware.aux", "P02");
		break;

	case 3:

		dstate_setinfo("ups.firmware.aux", "P03");
		break;

	case 8:

		dstate_setinfo("ups.firmware.aux", "P08");
		break;

	case 9:

		dstate_setinfo("ups.firmware.aux", "P09");
		break;

	case 10:

		dstate_setinfo("ups.firmware.aux", "P10");
		break;

	case 14:

		dstate_setinfo("ups.firmware.aux", "P14");
		break;

	case 31:

		dstate_setinfo("ups.firmware.aux", "P31");
		break;

	case 99:

		dstate_setinfo("ups.firmware.aux", "P99");
		break;
...
/* voltronic_fault()
 * Check if a fault is reported by the UPS and its type.
 * note: call this function only if a fault is found in 12bit flag of QGS, otherwise you'll get a fake reply */
static int	voltronic_fault(void)
{
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for fault type rejected by the UPS", __func__);
		return -1;
	}
	...
	/* Check needed since this function may be called in voltronic_initinfo() by voltronic_status() before the check for protocol */
	if (!dstate_getinfo("ups.firmware.aux") && voltronic_protocol()) {
		upsdebugx(2, "%s: failed to get protocol from the UPS", __func__);
		return -1;
	}

	/* Deleting from buf unneeded values (short status) */
	strtok(buf, " ");
	...
	if ((strspn(buf+1, "0123456789ABC") != 2) || ((buf[1] != '1') && (strspn(buf+2, "0123456789") != 1))) {
		...
		if (strpbrk(buf+2, "ABC")) {
			...
			case 7:
				...
				snprintfcat(status_buf, sizeof(status_buf), " OVER");
				break;
...
/* voltronic_warning()
 * Check if a warning is reported by the UPS and its type */
static int	voltronic_warning(void)
{
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for warning type rejected by the UPS", __func__);
		return -1;
	}
	...
	if (strspn(buf+1, "01") != (strcspn(buf, "\r\n")-1)) {
		upsdebugx(2, "%s: invalid reply from the UPS [%s]", __func__, strtok(buf+1, "\r\n"));
		return -1;
	}
	...
	for (i = 1; i <= ((int)strcspn(buf, "\r\n") - 1); i++) {
		...
		if (buf[i] == '1') {
			...
			default:
				...
				snprintf(warn, sizeof(warn), "Unknown warning from UPS [bit: #%02d]", i);
				u++;
				break;
			...
			if (status) {
				...
				/* Don't store status, if already set */
				if (!strstr(status_buf, status)) {
			...
			/* Known warnings */
				...
				if (strlen(warns) > 0) {
					...
					snprintfcat(bitwarns, sizeof(bitwarns), ", #%02d", i);
	...
	if (strlen(warns) > 0) {
		...
		/* Both known and unknown warnings */
		if (strlen(unk) > 0) {
	...
	/* If grand total of previous alarms + warnings doesn't exceed value of alarm_buf (=ST_MAX_VALUE_LEN).. */
	if ((ST_MAX_VALUE_LEN - strlen(alarm_buf)) > strlen(warns)) {

		/* ..then be explicit.. */
		snprintfcat(alarm_buf, sizeof(alarm_buf), " %s", warns);
...
/* voltronic_mode()
 * Check working mode reported by the UPS */
static int	voltronic_mode(void)
{
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for mode rejected by the UPS", __func__);
		return -1;
	}
	...
	/* Don't store alarm, if already set */
	if (alarm && !strstr(alarm_buf, alarm)) {
		snprintfcat(alarm_buf, sizeof(alarm_buf), " %s", alarm);
	}

	/* Remove removestatus from status, if needed */
	if (removestatus && strstr(status_buf, removestatus)) {

		/* Case: status = something1+removestatus[+something2] or removestatus+something2 */
		if (strlen(status_buf) != strlen(removestatus)) {

			/* Case: status = something1+removestatus[+something2] -> copying something1 to buffer (status_buffer) and then concat something2 */
			if (strstr(status_buf, removestatus) != status_buf) {
		...
		/* Case: status == removestatus -> copying in status a blank buffer (status_buffer) */
		strcpy(status_buf, status_buffer);

	}

	/* Don't store status, if already set */
	if (status && !strstr(status_buf, status)) {
		snprintfcat(status_buf, sizeof(status_buf), " %s", status);
	}
...
static int	voltronic_status(void)
{
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for status rejected by the UPS", __func__);
		return -1;
	}
	...
	if (strspn(val, "01") != 12) {
		upsdebugx(2, "%s: invalid status [%s]", __func__, val);
		return -1;
	}
	...
	/* UPS Type: 00 -> Offline | 01 -> Line-interactive | 10 -> Online */
	if (!strncmp(val, "00", 2)) {
	...
	if (!mode) {
		...
		/* Only on startup we'll try MAXTRIES times */
		for (i = 1; i <= MAXTRIES; i++) {
			...
			/* If OK -> the next time we'll try only 1 time.. */
			mode = 0;
		...
		if (ret) {
			...
			/* ..otherwise the next times we'll skip */
			mode = -1;
		}
	...
	/* Removing leading white space from alarm_buf */
	alarm_set(strlen(alarm_buf) > 0 ? alarm_buf+1 : "");

	/* Removing leading white space from status_buf */
	status_set(strlen(status_buf) > 0 ? status_buf+1 : "");
...
/* voltronic_capability()
 * Check whether or not the UPS has some capabilities */
static int	voltronic_capability(void)
{
	char	buf[SMALLBUF], *enabled, *disabled, *last = NULL;

	/* Total options available: 23, only those whom the UPS is capable of are reported as Enabled or Disabled
	 * > [QFLAG\r]
	 * < [(EpashcDbroegfl\r]
	 *    0123456789012345
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for capability rejected by the UPS", __func__);
		return -1;
	}
	...
	/* The following options are treated as variables configurable in ups.conf.
	 * If you want to change the value of these options, simply add the corresponding variable & value in ups.conf.
	 * The driver will check on startup and then tell the UPS to change them. [see also voltronic_check()] */
...
/* voltronic_check()
 * Try to change UPS capability according to user configuration in ups.conf
 * This function returns:
 *  - 0 on success
 *  - rtn if provided value in ups.conf isn't acceptable and for errors while changing options in the UPS
 *  - 1 if the UPS isn't OFF and it can't reset options
 *  - -1 on other errors */
static int	voltronic_check(void)
{
	const struct {
		const char	*type;	/* Name of the option, used also to get the value from ups.conf */
		const char	*acr;	/* The corresponding acronym used by the UPS */
		const char	*match;	/* The value reported by the UPS */
	} capability[] = {
	...
	/* If reset_to_default is set in ups.conf, the function will skip other steps */
	if (testvar("reset_to_default")) {
		...
		if (!strncasecmp(buf, "(ACK", 4)) {
			...
			upslogx(LOG_INFO, "UPS capability: the UPS successfully reset options to safe default values");
			check++;

		} else {

			upslogx(LOG_ERR, "UPS capability: the UPS failed to reset options to safe default values");
			return -1;
		...
		for (i = 0; capability[i].type; i++) {
			...
			/* Skipping to the next option if this one is not set in ups.conf */
			if (!val) {
				continue;
			}
			...
			if (!strncasecmp(buf2, "(ACK", 4)) {
				...
				upslogx(LOG_INFO, "UPS capability: the UPS changed successfully [%s] option from [%s] to [%s]", capability[i].type, match, val);
				check++;

			} else {

				upslogx(LOG_ERR, "UPS capability: the UPS failed to change [%s] option from [%s] to [%s]", capability[i].type, match, val);
				rtn++;
	...
	/* No options have been changed -> no need to update capability */
	if (!check) {
		return rtn;
	}
 ...
 * Then try to change them according to values provided by user in ups.conf
 * This functions returns:
 *  - -1 on errors
 *  - 0 on success
 *  - -2 if ECO Mode is not supported by the UPS
 *  - rtn>0 if non-blocking errors occur */
static int	voltronic_eco(void)
{
	struct {
		struct {
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for maximum and minimum voltage for ECO Mode rejected by the UPS", __func__);
		return -1;
	}
	...
	/* If reset_to_default is set in ups.conf, the function won't change voltage limits */
	if (testvar("reset_to_default")) {
		...
		/* Try to change values according to vars set in ups.conf */
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for maximum and minimum frequency for ECO Mode rejected by the UPS", __func__);
		return -1;
	}
	...
	/* Since query for ratings (QRI) is not mandatory to run this driver, skip next steps if we can't get the value of output frequency nominal */
	if (ofn == -1) {
	...
	/* If reset_to_default is set in ups.conf, the function won't change frequency limits */
	if (testvar("reset_to_default")) {
...
/* voltronic_bypass()
 * Check voltage/frequency limits for Bypass Mode (if the UPS is overload and input voltage/frequency are within acceptable range, it'll enter bypass mode or bypass mode can be set through front panel).
 * Then try to change them according to values provided by user in ups.conf
 * This function returns:
 *  - -1 on errors
 *  - 0 on success
 *  - -2 if Bypass Mode is not supported by the UPS
 *  - rtn>0 if non-blocking errors occur */
static int	voltronic_bypass(void)
{
	struct {
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for maximum and minimum voltage for Bypass Mode rejected by the UPS", __func__);
		return -1;
	}
	...
	/* Since query for ratings (QMD) is not mandatory to run this driver, skip next steps if we can't get the value of input voltage nominal */
	if (ivn == -1) {
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for maximum and minimum frequency for Bypass Mode rejected by the UPS", __func__);
		return -1;
	}
...
/* voltronic_batt_numb()
 * Change value of number of batteries according to value set by user in ups.conf
 * Note: changing number of batteries will change the UPS's estimation on battery charge/runtime
 * This function returns:
 * - 0 on success
 * - 1 if provided value in ups.conf isn't acceptable
 * - -1 on other errors */
static int	voltronic_batt_numb(void)
{
	...
	struct {
		int		act;	/* Actual number of batteries as reported by the UPS */
	...
	/* Skipping if there's no option set in ups.conf or if we can't get number of batteries from the UPS */
	if (!batt_numb.conf || !battery_number) {
		return 0;
	}
...
/* voltronic_batt_packs()
 * Change value of number of battery packs to value set by user in ups.conf
...
static int	voltronic_batt_packs(void)
{
	...
	struct {
		int		act;	/* Actual number of battery packs as reported by the UPS */
	...
	/* Skipping if there's no option set in ups.conf or if we can't get number of battery packs from the UPS */
	if (!batt_packs.conf || !dstate_getinfo("battery.packs")) {
		return 0;
	}
...
/* voltronic_p31b()
 * P31 UPSes only: query UPS for type of battery and try to change it to value set in ups.conf
 * This function returns:
 *  - 0 on success
 *  - 1 if provided value in ups.conf isn't acceptable
 *  - -1 on other errors */
static int	voltronic_p31b(void)
{
	...
	struct {
		int		act;	/* Actual type of battery as reported by the UPS */
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for battery type rejected by the UPS", __func__);
		return -1;
	}
	...
	if ((buf[1] != '0') || (strspn(buf+2, "012") != 1)) {
		upsdebugx(2, "%s: invalid battery type reported by the UPS [%s]", __func__, strtok(buf+1, "\r\n"));
		return -1;
	}

	batt_type.act = atoi(buf+1);

	upslogx(LOG_INFO, "Actual battery type as reported by the UPS: %s", batt_type.op[batt_type.act]);
...
/* voltronic_p31g()
 * P31 UPSes only: query UPS for device grid working range type and try to change it to value set in ups.conf
 * This function returns:
 *  - 0 on success
 *  - 1 if provided value in ups.conf isn't acceptable
 *  - -1 on other errors */
static int	voltronic_p31g(void)
{
	...
	struct {
		int		act;	/* Actual device grid working range as reported by the UPS */
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for device grid working range rejected by the UPS", __func__);
		return -1;
	}
	...
	if ((buf[1] != '0') || (strspn(buf+2, "01") != 1)) {
		upsdebugx(2, "%s: invalid device grid working range reported by the UPS [%s]", __func__, strtok(buf+1, "\r\n"));
		return -1;
	}

	work_range.act = atoi(buf+1);

	upslogx(LOG_INFO, "Actual device grid working range as reported by the UPS: %s", work_range.type[work_range.act]);
...
static int	voltronic_ratings1(void)
{
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for rated informations #1 rejected by the UPS", __func__);
		return -1;
	}
...
static int	voltronic_ratings2(void)
{
	...
	const struct {
		...
		{ "output.powerfactor",		"%.1f" },	/* The UPS reports a value expressed in % so -> output.powerfactor*100 e.g. opf = 0,8 -> ups = 80 */
		{ "input.phases",		"%.0f" },
		{ "output.phases",		"%.0f" },
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for rated informations #2 rejected by the UPS", __func__);
		return -1;
	}
	...
	for (i = 0, index = 1; information[i].var; index += information[i++].len+1) {
		...
		if (strchr(val, '#')) {
			...
			/* If the UPS report a ##..# value we'll log it but we don't store it in a NUT variable */
			if (strspn(val, "#") == strlen(val)) {
				upslogx(LOG_INFO, "The UPS reported a non-significant value for %s [%s]", information[i].var, val);
				continue;
			}
			...
			/* If the UPS report a blank value we'll log it but we don't store it in a NUT variable */
			if (strspn(val, " ") == strlen(val)) {
				upslogx(LOG_INFO, "The UPS reported a non-significant value for %s [%s]", information[i].var, val);
				continue;
			}
	...
	for (i = 0, val2 = strtok_r(buf+25, " ", &last); infos[i].var; i++, val2 = strtok_r(NULL, " /\r\n", &last)) {
		...
		/* Skipping if info has been already obtained */
		if (dstate_getinfo(infos[i].var)) {
			continue;
		}
...
static int	voltronic_manufacturer(void)
{
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for manufacturer rejected by the UPS", __func__);
		return -1;
	}
	...
	if (strchr(buf+1, '#')) {
		...
		/* If the UPS report a ##..# manufacturer we'll log it but we don't store it in device.mfr */
		if (strspn(buf+1, "#") == (strlen(buf) - 2)) {
		...
		/* If the UPS report a blank manufacturer we'll log it but we don't store it in device.mfr */
		if (strspn(buf+1, " ") == (strlen(buf) - 2)) {
			upslogx(LOG_INFO, "The UPS reported a non-significant manufacturer [%s]", strtok(buf+1, "\r\n"));
			return 0;
		}
...
/* voltronic_firmware()
 * Query for the UPS firmware version */
static int	voltronic_firmware(void)
{
	char	buf[SMALLBUF];
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for firmware rejected by the UPS", __func__);
		return -1;
	}
	...
	/* If the UPS report a 00..0 firmware we'll log it but we don't store it in ups.firmware */
	if (strspn(buf+1, "0.") == (strlen(buf) - 8)) {
		upslogx(LOG_INFO, "The UPS reported a non-significant firmware [%s]", strtok(buf+7, "\r\n"));
		return 0;
	}
...
/* voltronic_serial()
 * Query for the UPS serial number */
static int	voltronic_serial(void)
{
	char	buf[SMALLBUF];
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for serial rejected by the UPS", __func__);
		return -1;
	}
	...
	if (strspn(buf+1, "0") == (strlen(buf) - 2)) {
		upslogx(LOG_INFO, "The UPS reported a non-significant serial [%s]", strtok(buf+1, "\r\n"));
		return 0;
	}
 ...
 * Old way - Retained as a double-check control for infos not get with newer queries */
static int	voltronic_vendor(void)
{
	const struct {
		const char	*var;
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for vendor informations rejected by the UPS", __func__);
		return -1;
	}
	...
	for (i = 0, index = 1; information[i].var; index += information[i++].len+1) {
		...
		/* Skipping if info has been already obtained or if val is blank */
		if (dstate_getinfo(information[i].var) || ((int)strspn(val, " ") == information[i].len)) {
			continue;
		}
...
/* voltronic_ratings3()
 * Query UPS for ratings #3
 * Old way - Retained as a double-check control and for infos not get with newer queries */
static int	voltronic_ratings3(void)
{
	const struct {
		const char	*var;
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for rated informations #3 rejected by the UPS", __func__);
		return -1;
	}
	...
	for (i = 0, val = strtok_r(buf+1, " ", &last); rating[i].var; i++, val = strtok_r(NULL, " \r\n", &last)) {
		...
		/* Skipping if info has been already obtained */
		if (dstate_getinfo(rating[i].var)) {
			continue;
		}
...
/* voltronic_outlet(number of outlet, whether voltronic_initinfo() or not)
 * Query UPS for outlets status and delay time and try to change it to value set in ups.conf
 * This function returns:
 *  - -1 if the UPS can't switch outlets
 *  - -2 if outlet #outlet_numb is not programmable
 *  - 2 on delaytime errors if provided value in ups.conf isn't acceptable
 *  - 1 on other errors
 *  - 0 on success */
static int	voltronic_outlet(const int outlet_numb, const int init)
{
	...
	struct {
		int	act;	/* Actual delay for outlet "outlet_numb" as reported by the UPS */
	...
	/* If outlet.0.switchable is unset -> the UPS hasn't reported 'j' in the capability flag.. */
	if (!dstate_getinfo("outlet.0.switchable")) {

		upslogx(LOG_INFO, "UPS has no programmable outlet.");
		return -1;

	/* ..if set to "no" -> the UPS has 'j' capability, but it's disabled */
	} else if (!strcmp(dstate_getinfo("outlet.0.switchable"), "no")) {
	...
	/* If the UPS replies (NAK the outlet is not programmable */
	if (!strncasecmp(buf, "(NAK", 4)) {

		/* Log only if in voltronic_initinfo() */
		if (init == 1) {
	...
	case '1':
		...
		dstate_setinfo(buf2, "yes");
		dstate_setinfo(buf3, "on");
		break;

	case '0':

		dstate_setinfo(buf2, "yes");
		dstate_setinfo(buf3, "off");
		break;
	...
	/* Quit function if not in voltronic_initinfo() */
	if (init == 0) {
		return 0;
	}
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		...
		upslogx(LOG_INFO, "Delay time before programmable outlet %.1d shuts down the load when on battery mode not set in the UPS.", outlet_numb);
		delay.act = -1;
	...
	/* Skipping if there's no option set in ups.conf */
	if (!val) {
		return 0;
	}
...
/* voltronic_batt_low()
 * Query UPS for actual battery low voltage and try to change it to value set in ups.conf
...
static int	voltronic_batt_low(void)
{
	...
	struct {
		int	act;	/* Actual value for battery low voltage as reported by the UPS */
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for battery low voltage rejected by the UPS", __func__);
		return -1;
	}
 ...
 *  - -1 on errors
 *  - 0 on success
 *  - 1 if not in three-phase
 *  - rtn<0 on other errors */
static int	voltronic_multi(void)
{
	/* > [Q3**\r]
	...
	/* P03 = 3-phase input/1-phase output UPSes */
	} else if (!strcmp(dstate_getinfo("ups.firmware.aux"), "P03")) {
		...
		/* As far as I know only P03/P09/P10 UPSes have multi-phase capability */
		return 1;
	...
	/* Quit function if not multi-phase */
	if ((atoi(dstate_getinfo("input.phases")) < 2) && (atoi(dstate_getinfo("output.phases")) < 2)) {
	...
	for (i = 0; multiquery[i].cmd; i++) {
		...
		/* Not all queries are accepted by multi-phase UPSes, so if a query is rejected we'll skip it but we won't report an error (rtn--) */
		if (!strncasecmp(buf, "(NAK", 4)) {
			upsdebugx(2, "%s: %s rejected by the UPS", __func__, multiquery[i].desc);
			continue;
		}
		...
		for (j = 0, val = strtok_r(buf+1, " ", &last); multiquery[i].type[j]; j++, val = strtok_r(NULL, " \r\n", &last)) {
			...
			/* P09 = two-phase input/output UPSes */
			if (!strcmp(dstate_getinfo("ups.firmware.aux"), "P09") && strchr(multiquery[i].type[j], '3')) {
				continue;
			}
...
/* voltronic_load_level()
 * Query UPS for last seen minimum/maximum load level */
static int	voltronic_load_level(void)
{
	char	buf[SMALLBUF], *val, *last = NULL;
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for last seen maximum and minimum load level rejected by the UPS", __func__);
		return -1;
	}
...
/* voltronic_phase()
 * Query UPS for actual input/output phase angles and try to change output phase angle to value set in ups.conf
 * This function returns:
 *  - 0 on success
 *  - 1 if provided value in ups.conf isn't acceptable
 *  - -1 on other errors */
static int	voltronic_phase(void)
{
	...
	struct {
		int	input;	/* Actual input phase angle as reported by the UPS */
		struct {
			int	act;	/* Actual output phase angle as reported by the UPS */
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for input and output phase angle rejected by the UPS", __func__);
		return -1;
	}
	...
	if (strspn(val, "0123456789") != strlen(val)) {
		upslogx(LOG_ERR, "Output phase angle: non numerical value [%s]", val);
		return 1;
	}
...
static int	voltronic_parallel(void)
{
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for master/slave for parallel UPSes rejected by the UPS", __func__);
		return -1;
	}
	...
	upslogx(LOG_INFO, "This UPS is *%s* in a system of UPSes in parallel", atoi(buf+1) == 1 ? "master" : "slave");
...
/* voltronic_qbdr()
 * TESTING NEEDED
 * I don't really know what this query is meant for (Baud Rate?) */
static int	voltronic_qbdr(void)
{
	char	buf[SMALLBUF];
	...
	/* Exec this function only if testing is invoked in ups.conf */
	if (!testvar("testing")) {
		return 0;
	}
	...
	if (!strncasecmp(buf, "(NAK", 4)) {
		upsdebugx(2, "%s: query for QBDR rejected by the UPS", __func__);
		return -1;
	}
...
/* voltronic_instcmd(cmdname, extra)
...
static int	voltronic_instcmd(const char *cmdname, const char *extra)
{
	...
	for (i = 0; instcmd[i].cmd; i++) {
		...
		/* If a command is invalid or rejected, the UPS will reply (NAK, on success it'll reply (ACK */
		voltronic_command(buf, buf2, sizeof(buf2));
	...
	if (!strcasecmp(cmdname, "beeper.toggle")) {
		...
		/* Provided that the beeper is not disabled:
		 *  - if the UPS is beeping then we can call BZOFF
		 *  - if we previusly called BZOFF we can call BZON */

		/* If the UPS can disable/enable alarm (from UPS capability) */
		if (alarm_control) {
			...
			/* Beeper disabled */
				...
				upslogx(LOG_INFO, "instcmd: the beeper is already disabled");
				return STAT_INSTCMD_FAILED;

			}

		/* The UPS lacks the ability to disable/enable alarm (from UPS capability) */
		} else {
		...
		if (extra && strspn(extra, "0123456789") != strlen(extra)) {
			...
			upslogx(LOG_ERR, "instcmd: battery test time - non numerical value [%s]", extra);
			return STAT_INSTCMD_FAILED;
		...
		if ((delay < 12) || (delay > 5940)) {
			...
			upslogx(LOG_ERR, "instcmd: battery test time '%d' out of range [12..5940] seconds", delay);
			return STAT_INSTCMD_FAILED;
...
void	voltronic_makevartable(void)
{
	...
	/* Vars for programmable outlets control */
...
/* voltronic_initups()
 * Check if value for on/off delay are OK */
...
/* voltronic_initinfo()
 * Find supported UPS and try to get/change its infos/settings */
void	voltronic_initinfo(void)
{
	...
	upsdebugx(1, "Trying to get status and protocol from the UPS...");

	/* Querying UPS for status */
	for (retry = 1; retry <= MAXTRIES; retry++) {

		ret = voltronic_status();

		if (ret < 0) {
			upsdebugx(1, "Status read %d failed", retry);
			continue;
		}

		upsdebugx(1, "Status read in %d tries", retry);
		break;

	}

	/* Querying UPS for protocol */
	for (retry = 1; retry <= MAXTRIES; retry++) {

		pro = voltronic_protocol();

		if (pro < 0) {
			upsdebugx(1, "Protocol read %d failed", retry);
			continue;
		}

		upsdebugx(1, "Protocol read in %d tries", retry);
		break;
	...
	/* #3 - voltronic_multi() */
	/* note: voltronic_multi() needs voltronic_ratings2() */
	for (retry = 1; retry <= MAXTRIES; retry++) {
		...
		if (ret > 0) {
			...
			/* If the UPS is not multi-phase, we'll skip voltronic_multi() in upsdrv_updateinfo() */
			multi = 0;
	...
	for (retry = 1; retry <= MAXTRIES; retry++) {
		...
		for (i = 1; i <= MAXTRIES; i++) {
			...
			/* note: voltronic_check() needs voltronic_capability() (& voltronic_mode() for resetability) */
			pro = voltronic_check();
	...
	/* #1 - voltronic_batt_low() */
	/* note: voltronic_batt_low() needs voltronic_ratings1() / voltronic_ratings2() */
	for (retry = 1; retry <= MAXTRIES; retry++) {

		ret = voltronic_batt_low();
	...
	/* #2 - voltronic_batt_numb() */
	/* note: voltronic_batt_numb() needs voltronic_battery() */
	for (retry = 1; retry <= MAXTRIES; retry++) {

		ret = voltronic_batt_numb();
	...
	/* #3 - voltronic_batt_packs() */
	/* note: voltronic_batt_packs() needs voltronic_battery() */
	for (retry = 1; retry <= MAXTRIES; retry++) {

		ret = voltronic_batt_packs();
	...
	/* #4 - voltronic_eco() */
	/* note: voltronic_eco() needs voltronic_ratings1() / voltronic_ratings2() */
	for (retry = 1; retry <= MAXTRIES; retry++) {
	...
	/* #5 - voltronic_bypass() */
	/* note: voltronic_bypass() needs voltronic_ratings1() & voltronic_ratings2() */
	for (retry = 1; retry <= MAXTRIES; retry++) {

		ret = voltronic_bypass();
	...
	for (i = 1, ret=0; i < 5 && ret != -1; i++) {
		...
		/* See explanation on 'Populating NUT vars..'
		 * To use in % (modulo) in upsdrv_updateinfo(): 1 -> 2, 2 -> 3, 3 -> 5, 4 -> 7 */
		outlet = outlet * ((ret == 1 || ret == -2) ? (i == 1 ? 2 : i == 2 ? 3 : i == 3 ? 5 : i == 4 ? 7 : 1) : 1);
	}

	/* #7 - voltronic_parallel() */
	for (retry = 1; retry <= MAXTRIES; retry++) {

		ret = voltronic_parallel();

		if (ret < 0) {
			upsdebugx(1, "Master/slave for UPSes in parallel: determining attempt #%d failed", retry);
			continue;
		}

		upslogx(LOG_INFO, "Master/slave for UPSes in parallel determined in %d tries", retry);
		break;

	}

	if (ret) {
		upslogx(LOG_DEBUG, "Error while determining master/slave for UPSes in parallel");
	}
	...
	/* For P31 UPSes */
	/* The following queries and commands work only on P31 UPSes */
	if (!strcmp(dstate_getinfo("ups.firmware.aux"), "P31")) {
	...
	/* == Testing == */
	/* The following function need testing with UPSes, since I don't know what the UPS will reply to the query */
	if (testvar("testing")) {
...
void	upsdrv_updateinfo(void)
{
	...
	if (
		voltronic_status() ||
		(battery && voltronic_battery()) ||
		(loadlevel && voltronic_load_level()) ||
		(multi && (voltronic_multi() < 0)) ||
		(
			(dstate_getinfo("outlet.0.switchable") && !strcmp(dstate_getinfo("outlet.0.switchable"), "yes")) ?
				(
					((outlet % 2) && voltronic_outlet(1, 0) > 0) ||
					((outlet % 3) && voltronic_outlet(2, 0) > 0) ||
					((outlet % 5) && voltronic_outlet(3, 0) > 0) ||
					((outlet % 7) && voltronic_outlet(4, 0) > 0)
				)
			: 1 == 0
		)
	) {
...
void	upsdrv_shutdown(void)
{
	...
	for (retry = 1; retry <= MAXTRIES; retry++) {
		...
		/* If you set stayoff in ups.conf when FSD arises the UPS will shutdown after *offdelay* seconds and won't return.. */
		if (testvar("stayoff")) {
		...
		/* ..otherwise (standard behaviour) the UPS will shutdown after *offdelay* seconds and then turn on after *ondelay* minutes (if mains meanwhile returned) */
		} else {
