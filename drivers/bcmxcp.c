...
bool_t init_command(int size)
{
	...
	else
	{
		...
		upsdebugx(3, "Number of commands implemented in ups %" PRIssize "", res);
		iIndex++;
		res = answer[iIndex]; /* Entry length - bytes reported for each command */
		iIndex++;
		upsdebugx(5, "bytes per command %" PRIssize "", res);
...
unsigned char init_outlet(unsigned char len)
{
	...
	if (res <= 0)
		...
		upsdebugx(1, "init_outlet(%i), res=%" PRIssize "", len, res);
...
void upsdrv_initinfo(void)
{
	...
	if (outlet_block_len > 8) {
		...
		for (res = 1 ; (unsigned int)res <= (unsigned int)len ; res++) {
			snprintf(outlet_name, sizeof(outlet_name) - 1, "outlet.%" PRIssize ".shutdown.return", res);
			dstate_addcmd(outlet_name);
			snprintf(outlet_name, sizeof(outlet_name) - 1, "outlet.%" PRIssize ".load.on", res);
			dstate_addcmd(outlet_name);
			snprintf(outlet_name, sizeof(outlet_name) - 1, "outlet.%" PRIssize ".load.off", res);
			dstate_addcmd(outlet_name);
		}
	}
