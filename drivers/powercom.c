...
static int ups_getinfo(void)
{
	...
	if (ser_send_char (upsfd, SEND_DATA) != 1) {
		...
		if (c != (ssize_t)types[type].num_of_bytes_from_ups) {
			upslogx(LOG_NOTICE, "data receiving error (%" PRIssize " instead of %d bytes)", c, types[type].num_of_bytes_from_ups);
			dstate_datastale();
			return 0;
		} else
			upsdebugx(5, "Num of bytes received from UPS: %" PRIssize "", c);
	}
