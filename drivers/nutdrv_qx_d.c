/* nutdrv_qx_d.c - Subdriver for D protocol based UPSes
 *
 * Copyright (C)
 ...
 *
 * NOTE:
 * This subdriver implements the same protocol as the one used by the 'megatec/old' subdriver minus the vendor (I) and ratings (F) queries.
 * In the claim function:
 * - it doesn't even try to get 'vendor' informations (I)
 * - it checks only status (D), through 'input.voltage' variable
 ...
 * - the UPS replies a D-compliant answer (i.e. not necessary filled with all of the D-required data, but at least of the right length and with not available data filled with some replacement character)
...
#define D_VERSION "D 0.01"

/* qx2nut lookup table */
static item_t	d_qx2nut[] = {
